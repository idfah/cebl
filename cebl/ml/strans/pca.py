...
class PrincipalComponentsAnalysis(STrans):
    ...
    def plotMags(self, standardize=True, ax=None, *args, **kwargs):
        result = {}
        if ax is None:
            fig = plt.figure()
            result['fig'] = fig
        ...
        bars = plt.bar(sep, mags, *args, **kwargs)
        result['bars'] = bars
...
def demoPCA2d():
    ...
    ax.scatter(s[:,0], s[:,1])
    ax.arrow(0.0, 0.0, pca.wInv[0,0]/pca.mags[0], pca.wInv[0,1]/pca.mags[0], head_width=0.05, head_length=0.1, color='red')
    ax.arrow(0.0, 0.0, pca.wInv[1,0]/pca.mags[1], pca.wInv[1,1]/pca.mags[1], head_width=0.05, head_length=0.1, color='red')
    ax.grid()
